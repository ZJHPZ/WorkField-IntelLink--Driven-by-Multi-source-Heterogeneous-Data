        ...
        <div class="relative z-[3]">
          ...
          <div class="flex items-center gap-6">
            ...
            <div class="flex-1 grid grid-cols-2 gap-2">
              <div class="p-3 text-center rounded-sm" style="background:rgba(16,185,129,0.04);border:1px solid rgba(16,185,129,0.1)"><div class="data-giant text-xl text-mint-500">{{ totalOverlap }}</div><div class="text-xs font-mono tracking-widest mt-0.5" :style="{color:'var(--text-muted)'}">TRANSFER</div></div>
              <div class="p-3 text-center rounded-sm" style="background:rgba(244,63,94,0.04);border:1px solid rgba(244,63,94,0.1)"><div class="data-giant text-xl text-rose-500">{{ totalGap }}</div><div class="text-xs font-mono tracking-widest mt-0.5" :style="{color:'var(--text-muted)'}">NEW SKILLS</div></div>
              <div class="p-3 text-center rounded-sm" style="background:rgba(99,102,241,0.04);border:1px solid rgba(99,102,241,0.1)"><div class="data-giant text-xl text-brand-500">{{ estimatedMonths }}M</div><div class="text-xs font-mono tracking-widest mt-0.5" :style="{color:'var(--text-muted)'}">EST. TIME</div></div>
              <div class="p-3 text-center rounded-sm" style="background:rgba(6,182,212,0.04);border:1px solid rgba(6,182,212,0.1)"><div class="data-giant text-xl text-cyan-500">{{ marketDemand }}</div><div class="text-xs font-mono tracking-widest mt-0.5" :style="{color:'var(--text-muted)'}">DEMAND</div></div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="flex flex-col lg:flex-row items-center gap-6">
          <div class="shrink-0 relative" style="width:300px;height:240px">
            <svg viewBox="0 0 300 240" class="w-full h-full">
              <circle cx="100" cy="120" r="70" fill="rgba(99,102,241,0.04)" stroke="#818cf8" stroke-width="2" stroke-dasharray="6 3" opacity="0.6"/>
              <circle cx="200" cy="120" r="70" fill="rgba(6,182,212,0.04)" stroke="#06b6d4" stroke-width="2" stroke-dasharray="6 3" opacity="0.6"/>
              <clipPath id="c2c"><circle cx="200" cy="120" r="70"/></clipPath>
              <circle cx="100" cy="120" r="70" fill="rgba(16,185,129,0.08)" clip-path="url(#c2c)"/>
              ...
              <text v-for="(s,i) in fromOnlySkills" :key="'f'+i" :x="60" :y="110+(i-(fromOnlySkills.length-1)/2)*14" text-anchor="middle" font-size="9" fill="#818cf8" font-family="monospace">{{ s }}</text>
          ...
          <div class="flex-1 grid grid-cols-2 gap-3">
            <div class="panel-asymmetric p-3">
              ...
              <div class="space-y-1"><div v-for="s in overlapSkills" :key="s" class="text-xs font-mono p-1.5 rounded-sm flex items-center gap-2" style="background:rgba(16,185,129,0.04);color:var(--mint-500);border:1px solid rgba(16,185,129,0.15)"><span class="w-1 h-1 rounded-full bg-mint-500"></span>{{ s }}</div></div>
            </div>
            <div class="panel-asymmetric p-3">
...
const feasibilityBadgeBg = computed(()=>feasibilityScore.value>=70?'rgba(16,185,129,0.1)':feasibilityScore.value>=40?'rgba(245,158,11,0.1)':'rgba(244,63,94,0.1)')
...
const transitionPhases = computed(()=>{
  ...
  if(gaps.length>hg){phases.push({color:'#6366f1',title:'拓展辅助技能',duration:`${Math.ceil(estimatedMonths.value*0.2)}M`,description:`学习 ${gaps.slice(hg).join('、')}，完善能力矩阵`,tags:gaps.slice(hg)})}
